...
/// FPS counter.
/// </summary>
public class PointCloudGUI : MonoBehaviour
{
    ...
    public void Start()
    {
		...
		//path = Application.persistentDataPath + Path.DirectorySeparatorChar + "test.txt";
    }
	...
	{
		if (!File.Exists (path)) {
			string output = logString + "\t" + " Average depth (m): " + m_pointcloud.m_overallZ.ToString () +
				" Point count: " + m_pointcloud.m_pointsCount.ToString () + "\n";
